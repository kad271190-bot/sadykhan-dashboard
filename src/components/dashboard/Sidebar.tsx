...
function SadykhanLogo({ size = 36 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 100 100" fill="none">
      <path d="M 85 50 A 35 35 0 1 1 72 79" stroke="#3A9EA5" strokeWidth="11" strokeLinecap="round" fill="none"/>
      <path d="M 28 38 Q 50 22 75 34" stroke="#F5813F" strokeWidth="9" strokeLinecap="round" fill="none"/>
      <path d="M 24 52 Q 50 36 78 48" stroke="#F5813F" strokeWidth="8" strokeLinecap="round" fill="none" opacity="0.85"/>
      <path d="M 26 66 Q 52 50 76 62" stroke="#F5813F" strokeWidth="6.5" strokeLinecap="round" fill="none" opacity="0.65"/>
    </svg>
  )
}
...
export default function Sidebar({ active, onNav }: Props) {
  ...
  const [badges, setBadges] = useState<Record<string, number>>({ replies: 0, alerts: 0 })

  useEffect(() => {
    async function loadBadges() {
      const [replies, alerts] = await Promise.all([
        getUnreadCount('replies'),
        getUnreadCount('alerts'),
      ])
      setBadges({ replies, alerts })
    }
    loadBadges()
  }, [])

  const handleNav = async (section: string) => {
    onNav(section)
    if (badges[section] > 0) {
      setBadges(prev => ({ ...prev, [section]: 0 }))
      await markAsRead(section)
    }
  }
  ...
  const NAV = [
    ...
    { href: 'replies', icon: MessageSquare, label: 'Ответы' },
    { href: 'branches', icon: Building2, label: 'Филиалы' },
    { href: 'alerts', icon: AlertTriangle, label: 'Алерты' },
  ...
  return (
    <aside className={cn(
      ...
      <div className="flex items-center gap-2.5 px-3 py-4 border-b border-gray-100">
        <div className="shrink-0"><SadykhanLogo size={collapsed ? 28 : 36} /></div>
        {!collapsed && (
          <div>
